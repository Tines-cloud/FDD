 ...
 *
 * Templates support simple Mustache-style variable substitution: any occurrence of `{{variableName}}` is replaced with
 * the corresponding value from the supplied `variables` map.
 *
 * Template files are cached in memory after the first read to avoid repeated I/O on the classpath.
 */
@Service
class PromptTemplateServiceImpl : PromptTemplateService {
    ...
    /**
     * Load a template from `classpath:ai/[templateName]` and perform variable substitution.
     *
     * @param templateName File name relative to the `ai/` resource directory (e.g. `drift-analysis-system.txt`).
